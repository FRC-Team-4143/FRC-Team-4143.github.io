...
That model can only use the desired setpoint for its calculations since open loop control types dont have feedback.
...
For most of our applications using feed forward control, its just a simple % of the max output. 
...
For example if we want an intake to spin fast enough to pick up a game piece, we can just set the motor to 60% output and see if works and adjust from there.
```java
intakeMotor.set(0.6);
```

...
- Searching:
    - You can search for instances of text in a file by pressing `Ctrl + F`. 
    - You can also use `Ctrl + Shift + F` (or the magnifying glass on the left) to search across all files in the project. This is useful for finding where a variable or function is used throughout the codebase.
...
## Arrays
Similar to how we would declare and define a variable, we can do the same with an array. However, after we decide the datatype, we have to add square brackets `[]` to indicate we are creating an array.
```java
int[] int_array;
double[] double_array;
String[] string_array;
...
// indexes {0, 1, 2, 3}
```
That's all good, but we still need to be able to actually reference the element now that we know the index. You can access the element at a given `index` by using `array_name[index]`.
```java
int[] team_number = {4, 1, 4, 3};

...
## Comments
- All methods should be commented following java doc styling. Make sure to include any parameters or return members.
...
- Variables belonging to a class ***(or members)*** should have a trailing underscore `_`. Examples are shown below:
    ...
    float error_;
    ```
- Variables that are constant should be named using `SCREAMING_SNAKE_CASE`. They should also use the `final` keyword.
    ```java
    final double ELEVATOR_HOME_POSITION;
    final int COUNTER_RESET;
    final boolean IS_MOTOR_INVERTED;
    final float I_TERM_ERROR_MAX;
    ```
...
## Classes
- Class names should be named using `UpperCamelCase`. Examples are shown below:
    ```java
    ...
    ```
- The opening curly brace `{` for a class should be on the same line as the class signature. Examples shown below:
    ```java
    // This is correct formatting

...
There are a few comparison operators each with their own behavior. Below is a table that shows the available comparison
...
## Logical Operators
What if we want to make a compound condition? Compare if one condition is true and another false? Both true? Luckily
our pal [George Boole](https://en.wikipedia.org/wiki/George_Boole) had us in mind when he created the concept called
[boolean algebra](https://en.wikipedia.org/wiki/Boolean_algebra). The fundamentals of boolean logic are three logical
operators: **NOT**, **AND**, and **OR**. Below are some "truth tables" which show the boolean output of comparing
...
if true> : <code if false>;` A quick example showing its power can be seen below.
```java
...
want it to perform the same action.
```java
switch (month){
    case "December":
    case "January":

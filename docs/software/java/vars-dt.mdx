...
You can create a variable with out assigned a value to it. You would do this is you plan on using it, but do not have a value to assign at creation. The syntax is `[type] [name]` which can be seen is examples below.
```java
int team_number;
double pi;
boolean robot_is_on;
...
It can be dangerous leaving a variable unassigned at creation. You may no that you can't read it until it has a value, but if other developers are unaware they may read from it before its assigned. This can cause the code to error or crash.
:::

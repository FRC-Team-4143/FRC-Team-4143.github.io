...
You can create a variable without assigning a value to it. You would do this if you plan on using it, but do not have a value to assign at creation. The syntax is `[type] [name]` which can be seen is examples below.
```java
int team_number;
double pi;
boolean robot_is_on;
